...
class TestTracker(TestBase):
    ...
    def setUp(self):
        ...
        self.tracker = self.createTracker(self.portal, "issue-tracker")

        self.addMember(
            "member1", "Member One", "member1@example.com", ["Member"], "2005-01-01"
        )
        self.addMember(
            "member2", "Member Two", "member2@example.com", ["Member"], "2005-01-01"
        )

    def testEditTracker(self):
        self.tracker.title = u"title"
        self.tracker.description = u"description"
        self.tracker.help_text = self.rich_text(u"help text")
        ...
        self.assertEqual(self.tracker.help_text.output, u"help text")
        self.assertEqual(
            self.tracker.available_areas,
            [{"id": u"area", "title": u"Area", "description": u"Issue area"}],
        )
...
class TestEmailNotifications(TestBase):
    ...
    def setUp(self):
        ...
        self.addMember(
            "member3", "Member Three", "member3@example.com", ["Member"], "2005-01-01"
        )
    ...
    def testGetTagsInUse(self):
        ...
        self.assertEqual(
            self.tracker.getTagsInUse(), [("A", 2), ("B", 2), ("C", 1), ("D", 1)]
        )
    ...
    def testSpecialCharacterIssueEmail(self):
        ...
        self.createResponse(issue, text="more accented vowels: ò ù")
        ...
        self.createResponse(
            issue, text=u"iso accented vowels: ò ù".encode("iso-8859-1")
        )
        sendResponseNotificationMail(issue)
...
class TestTrackerSearch(TestBase):
    ...
    def testGetFilteredIssesByRelease(self):
        ...
        issues = sorted(
            [b.getId for b in self.issuefolder.getFilteredIssues(release=u"2.0")]
        )
        self.assertEqual(issues, [u"1", u"2"])
        issues = [b.getId for b in self.issuefolder.getFilteredIssues(release=u"1.0")]
        issues.sort()
    ...
    def testGetFilteredIssesByArea(self):
        ...
        issues = sorted(
            [b.getId for b in self.issuefolder.getFilteredIssues(area=u"ui")]
        )
        self.assertEqual(issues, [u"1", u"2"])
        issues = [
            b.getId for b in self.issuefolder.getFilteredIssues(area=u"functionality")
        ]
        issues.sort()
        self.assertEqual(issues, [u"3"])
    ...
    def testGetFilteredIssesByIssueType(self):
        ...
        issues = sorted(
            [b.getId for b in self.issuefolder.getFilteredIssues(issueType=u"bug")]
        )
        self.assertEqual(issues, [u"1", u"2"])
        issues = [
            b.getId for b in self.issuefolder.getFilteredIssues(issueType=u"feature")
        ]
        issues.sort()
        self.assertEqual(issues, [u"3"])
    ...
    def testGetFilteredIssesBySeverity(self):
        ...
        issues = sorted(
            [b.getId for b in self.issuefolder.getFilteredIssues(severity=u"Medium")]
        )
        self.assertEqual(issues, [u"1", u"2"])
        issues = [
            b.getId for b in self.issuefolder.getFilteredIssues(severity=u"Critical")
        ]
        issues.sort()
        self.assertEqual(issues, [u"3"])
    ...
    def testGetFilteredIssesByTargetRelease(self):
        ...
        issues = sorted(
            [b.getId for b in self.issuefolder.getFilteredIssues(targetRelease=u"2.0")]
        )
        self.assertEqual(issues, [u"1", u"2"])
        issues = [
            b.getId for b in self.issuefolder.getFilteredIssues(targetRelease=u"1.0")
        ]
        issues.sort()
        self.assertEqual(issues, [u"3"])
    ...
    def testGetFilteredIssesByState(self):
        ...
        self.createIssue(self.tracker, assignee=u"member2")
        self.createIssue(self.tracker, assignee=u"member2")
        setRoles(self.portal, TEST_USER_ID, ["TrackerManager"])
        self.workflow.doActionFor(self.tracker["3"], "accept-unconfirmed")
        setRoles(self.portal, TEST_USER_ID, ["Member"])
        issues = sorted(
            [b.getId for b in self.issuefolder.getFilteredIssues(state="unconfirmed")]
        )
    ...
    def testGetFilteredIssesByCreator(self):
        ...
        issues = sorted(
            [b.getId for b in self.issuefolder.getFilteredIssues(creator="some_member")]
        )
        self.assertEqual(issues, ["1", "2"])
        issues = [
            b.getId
            for b in self.issuefolder.getFilteredIssues(creator="another_member")
        ]
        issues.sort()
        self.assertEqual(issues, ["3"])
    ...
    def testGetFilteredIssesByResponsible(self):
        ...
        issues = sorted(
            [
                b.getId
                for b in self.issuefolder.getFilteredIssues(responsible="manager1")
            ]
        )
        self.assertEqual(issues, ["1", "2"])
        issues = [
            b.getId for b in self.issuefolder.getFilteredIssues(responsible="manager2")
        ]
        issues.sort()
        self.assertEqual(issues, ["3"])
    ...
    def testGetFilteredIssesByTags(self):
        ...
        self.createIssue(self.tracker, tags=("B", "C"), assignee=u"member2")
        self.createIssue(self.tracker, tags=("A", "D"), assignee=u"member2")
        issues = sorted([b.getId for b in self.issuefolder.getFilteredIssues(tags="B")])
        self.assertEqual(issues, ["1", "2"])
        ...
        issues = [
            b.getId
            for b in self.issuefolder.getFilteredIssues(
    ...
    def testGetFilteredIssesComplex(self):
        ...
        issues = sorted(
            [b.getId for b in self.issuefolder.getFilteredIssues(text="foo", area="ui")]
        )
        self.assertEqual(issues, ["1"])

        issues = [
            b.getId
            for b in self.issuefolder.getFilteredIssues(area="ui", issueType="feature")
        ]
        issues.sort()
        self.assertEqual(issues, ["1"])
    ...
    def testSubjectTolerance(self):
        ...
        issues = [
            b.getId
            for b in self.issuefolder.getFilteredIssues(tags=dict(operator="and"))
        ]
        self.assertEqual(issues, ["1"])

        issues = [
            b.getId
            for b in self.issuefolder.getFilteredIssues(Subject=dict(operator="or"))
        ]
        self.assertEqual(issues, ["1"])
        ...
        # instead of dicts.
        tags = {"operator": "and"}
        issues = [b.getId for b in self.issuefolder.getFilteredIssues(tags=tags)]
        self.assertEqual(issues, ["1"])
        tags = {"operator": "or"}
        issues = [b.getId for b in self.issuefolder.getFilteredIssues(tags=tags)]
        self.assertEqual(issues, ["1"])
        ...
        issues = [b.getId for b in self.issuefolder.getFilteredIssues(tags=tags)]
        self.assertEqual(issues, ["1"])
        tags = {"query": ["A"], "operator": "and"}
        issues = [b.getId for b in self.issuefolder.getFilteredIssues(tags=tags)]
        self.assertEqual(issues, ["1"])
        tags = {"query": ["A", "B"], "operator": "and"}
        issues = [b.getId for b in self.issuefolder.getFilteredIssues(tags=tags)]
        self.assertEqual(issues, [])
        tags = {"query": ["A", "B"], "operator": "or"}
        issues = [b.getId for b in self.issuefolder.getFilteredIssues(tags=tags)]
        self.assertEqual(issues, ["1"])


class TestLinkDetection(TestBase):
    ...
    def testLinksToIssues(self):
        ...
        self.assertEqual(tracker.linkDetection(u"\xfanicode"), u"\xfanicode")
        ...
        self.assertEqual(
            ...
            '<a href="http://nohost/plone/issue-tracker/1">#1</a>',
        )
        self.assertEqual(
            tracker.linkDetection("Links to #1 and #2."),
            'Links to <a href="http://nohost/plone/issue-tracker/1">#1</a> and <a href="http://nohost/plone/issue-tracker/2">#2</a>.',
        )

        # We are not fooled by a non existing issue:
        self.assertEqual(
            tracker.linkDetection("Issue #1 and non-issue #3."),
            'Issue <a href="http://nohost/plone/issue-tracker/1">#1</a> and non-issue #3.',
        )

        # Issues that are added to a different tracker only show up
        # for that tracker:
        tracker2 = self.createTracker(self.portal, "tracker2")
        self.assertEqual(tracker2.linkDetection("#1"), "#1")
        self.createIssue(tracker2, title="1")
        self.assertEqual(
            tracker2.linkDetection("#1"),
            '<a href="http://nohost/plone/tracker2/1">#1</a>',
        )

        # A combination of unicode and a link number should be possible::
        self.assertEqual(
            tracker.linkDetection(u"\xfanicode text with a link to #1"),
            u'\xfanicode text with a link to <a href="http://nohost/plone/issue-tracker/1">#1</a>',
        )
    ...
    def testLinksToRevisions(self):
        ...
        self.assertEqual(tracker.linkDetection("r42"), '<a href="silly">r42</a>')
        ...
        self.assertEqual(
            ...
            'Issue <a href="http://nohost/plone/issue-tracker/1">#1</a> is fixed in <'
            'a href="https://github.com/collective/Products.Poi/commit/42">r42</a>.',
        )
    ...
    def testLinkBugs(self):
        ...
        self.assertEqual(
            link_bugs("#1", self.tracker, base_url="http://example.org/issues"),

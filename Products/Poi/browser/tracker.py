...
class IssueFolderView(BrowserView):
    ...
    def getSortedFilteredIssues(self, criteria=None, **kwargs):
        ...
        if criteria:
            ...
            if criteria.get("sort_on") == "assignee":
                ...
                results = [
                    dict(brain=r[0], slug=r[1])
                    for r in sorted(
                        (i for i in newresults), key=lambda x: x[1], reverse=sort_order
                    )
                ]
            # if sorting by Subject/tags, do it as a string
                ...
                results = self.getFilteredIssues(criteria, **kwargs)
                for i in results:
                    sorted_tags = sorted((y for y in i.Subject), key=lambda x: x)
    ...
    def getMyIssues(
        self,
        openStates=["unconfirmed", "open", "in-progress"],
        memberId=None,
        manager=False,
    ):
    ...
    def getOrphanedIssues(
        self, openStates=["unconfirmed", "open", "in-progress"], memberId=None
    ):
